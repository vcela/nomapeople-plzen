...
// Row actions are submitted via fetch() (see assets/admin.js) so the page
// never reloads — that would collapse whichever lesson-week <details> the
// admin had open. The X-Requested-With header is how we tell that apart from
// a plain (no-JS) form submit, which still needs the old redirect-and-render
// fallback. "Delete" here is a visual soft-delete only — deleted_at just gets
// set/cleared, the row is still shown (struck through) and nothing is lost;
// toggling back is intentional so a misclick is never final.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'toggle_delete') {
    if (csrf_verify($_POST['csrf'] ?? null)) {
        $id = (int) ($_POST['id'] ?? 0);
        if ($id > 0) {
            $stmt = db()->prepare('SELECT created_at, deleted_at FROM registrations WHERE id = ?');
            $stmt->execute([$id]);
            $row = $stmt->fetch();
            if ($row) {
                $newValue = $row['deleted_at'] ? null : date('Y-m-d H:i:s');
                db()->prepare('UPDATE registrations SET deleted_at = ? WHERE id = ?')->execute([$newValue, $id]);
                $mutatedBucket = lesson_bucket_date($row['created_at'], $lessonWeekday);
            }
        }
    }
    if (!$isAjax) admin_redirect('index.php');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'hard_delete') {
    if (csrf_verify($_POST['csrf'] ?? null)) {
        $id = (int) ($_POST['id'] ?? 0);
        if ($id > 0) {
            $stmt = db()->prepare('SELECT created_at FROM registrations WHERE id = ?');
            $stmt->execute([$id]);
            $row = $stmt->fetch();
            if ($row) {
...
if ($isAjax) {
    $groupPayload = null;
    if ($mutatedBucket !== null) {
        $bucketRows = $groups[$mutatedBucket] ?? ['active' => [], 'deleted' => []];
        $activeCount = count($bucketRows['active']);
        $groupPayload = [
            'activeCount' => $activeCount,
            'activeLabel' => czech_count_label($activeCount, 'přihlášený', 'přihlášení', 'přihlášených'),
            'totalCount' => $activeCount + count($bucketRows['deleted']),
        ];
    }
    json_response([
        'ok' => true,
        ...
        'nextEventEmails' => $nextEventEmails,
        'allEmails' => $allEmails,
    ]);
}
...
?>
<div class="card">
  <p style="margin-top:0;">Přihlášených na nejbližší lekci: <strong id="next-event-count"><?= $nextEventCount ?></strong></p>
  <details>
    <summary id="next-event-emails-summary" style="cursor:pointer; font-weight:700; color:#884858;">E-maily na nejbližší lekci (<?= count($nextEventEmails) ?>)</summary>
    <textarea id="next-event-emails" readonly rows="3" onclick="this.select();" style="width:100%; margin-top:10px; padding:10px; border:1.5px solid rgba(136,72,88,0.25); border-radius:10px; font-size:13px; font-family:inherit;"><?= esc(implode(', ', $nextEventEmails)) ?></textarea>
  </details>
  <details style="margin-top:14px;">
    <summary id="all-emails-summary" style="cursor:pointer; font-weight:700; color:#884858;">Všechny e-maily, co se kdy přihlásily (<?= count($allEmails) ?>)</summary>
    <textarea id="all-emails" readonly rows="4" onclick="this.select();" style="width:100%; margin-top:10px; padding:10px; border:1.5px solid rgba(136,72,88,0.25); border-radius:10px; font-size:13px; font-family:inherit;"><?= esc(implode(', ', $allEmails)) ?></textarea>
  </details>
</div>
      ...
      <tr<?= $r['deleted_at'] ? ' class="row-deleted"' : '' ?>>
        ...
        <td class="col-sticky-right">
          <div class="row-actions">
            <form method="post" action="index.php" class="row-action-form">
              <?php csrf_field(); ?>
            ...
            <form method="post" action="index.php" class="row-action-form" data-confirm="Opravdu trvale smazat záznam <?= esc($r['name']) ?>? Tuto akci nelze vrátit zpět.">
              <?php csrf_field(); ?>
